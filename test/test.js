...
const badSampleFeed = fs
  .readFileSync(path.join(testFilesPath, 'bc-sample-bad.xml'), 'utf8')
  .toString();
const sampleFeedWithNewFeedUrl = fs
  .readFileSync(path.join(testFilesPath, 'bc-sample-new-feed-url.xml'), 'utf8')
  .toString();
const sampleCustomTagsFeed = fs
  .readFileSync(path.join(testFilesPath, 'bc-sample-custom-tags.xml'), 'utf8')
  .toString();
const sampleOrderFeed = fs
  .readFileSync(path.join(testFilesPath, 'bc-sample-order.xml'), 'utf8')
  .toString();
const sampleOrderingFeed = fs
  .readFileSync(path.join(testFilesPath, 'replyall-sample-ordering.xml'), 'utf8')
  .toString();
...
describe('Parsing Local Feeds', function () {
  ...
  it('should parse a bad feed and return an error', function () {
    expect(podcastFeedParser.getPodcastFromFeed.bind(podcastFeedParser, badSampleFeed)).to.throw(
      ERRORS.parsingError
    );
  });
});
...
describe('Checking custom options', function () {
  ...
  it('should throw a requiredError because of missing required fields', function () {
    ...
    expect(
      podcastFeedParser.getPodcastFromFeed.bind(podcastFeedParser, sampleCustomTagsFeed, options)
